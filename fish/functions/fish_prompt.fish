...
function fish_prompt --description 'Write out the prompt'
    ...
    set -l bg_color 1e1e2e # catppuccin mocha
    set -l fg_color $fish_color_normal
    ...
    if test -n "$SSH_CONNECTION"
        set -l host_color $fish_color_host_remote
    end
    _append left_prompt (echo_color -b $bg_color -o $prompt_tty_opt $host_color (prompt_hostname))

    # tty
    set -l prompttty (tty | sed -e 's|/dev/||')
    _append left_prompt (echo_color -b $bg_color $fish_color_comment "<$prompttty>")

    # VCS
    set -l git (__fish_git_prompt)
    if test -n "$git"
        _append left_prompt $git
    end
    set -l hg (__fish_hg_prompt)
    if test -n "$hg"
        _append left_prompt $hg
    end
